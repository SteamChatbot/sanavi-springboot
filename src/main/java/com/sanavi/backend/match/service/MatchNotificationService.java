...
public class MatchNotificationService {
  ...
  public void sendMatchConfirmed(int matchId, int bidId) {
    log.info(
        "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} result=START",
        matchId,
        bidId);
    ...
    try {
      ...
      if (match == null) {
        log.warn(
            "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} result=SKIP reason=MATCH_NOT_FOUND",
            matchId,
            bidId);
        return;
      }
      ...
      if (bid == null) {
        log.warn(
            "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} result=SKIP reason=BID_NOT_FOUND",
            matchId,
            bidId);
        return;
      }
      ...
      if (requester == null) {
        log.warn(
            "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} request_user_id={} result=SKIP reason=REQUESTER_NOT_FOUND",
            matchId,
            bidId,
            match.getUserId());
        return;
      ...
      if (lawyer == null) {
        log.warn(
            "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} lawyer_id={} result=SKIP reason=LAWYER_NOT_FOUND",
            matchId,
            bidId,
            bid.getLawyerId());
        return;
      ...
      boolean requesterSent = sendToRequester(
          requester.getEmail(),
          requester.getUserId(),
          requester.getName(),
          bid.getLawyerName(),
      ...
      boolean lawyerSent = sendToLawyer(
          lawyer.getEmail(),
          lawyer.getUserId(),
          bid.getLawyerName(),
          match.getRequesterName(),
          requester.getPhone(),
          bid.getBidPrice(),
          confirmedAt,
          matchUrl,
      ...
      log.info(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} request_user_id={} lawyer_id={} requester_sent={} lawyer_sent={} result=COMPLETE",
          matchId,
          bidId,
          requester.getUserId(),
          lawyer.getUserId(),
      ...
      log.error(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} result=FAIL reason=UNEXPECTED_ERROR exception={}",
          matchId,
          bidId,
  ...
  private boolean sendToRequester(
    ...
    try {
      ...
      String body = buildRequesterBody(
          requesterName,
          lawyerName,
          lawyerPhone,
          bidPrice,
      ...
      log.info(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} recipient_type=REQUESTER target_user_id={} result=SUCCESS",
          matchId,
          bidId,
          requesterId);
      ...
      log.error(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} recipient_type=REQUESTER target_user_id={} result=FAIL exception={}",
          matchId,
          bidId,
          requesterId,
          e.getClass().getSimpleName());
  ...
  private boolean sendToLawyer(
    ...
    try {
      ...
      log.info(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} recipient_type=LAWYER target_user_id={} result=SUCCESS",
          matchId,
          bidId,
          lawyerId);
      ...
      log.error(
          "action=MATCH_NOTIFICATION_SEND target_type=match target_id={} bid_id={} recipient_type=LAWYER target_user_id={} result=FAIL exception={}",
          matchId,
          bidId,
          lawyerId,
          e.getClass().getSimpleName());
